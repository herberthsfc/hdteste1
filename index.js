...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=5511996237647:+55 (62) 99663-8900\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '/'
blocked = []
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '[ ⏳ ]  Carregando...',
				success: '[ ✔️ ]  Concluido com Sucesso!',
				levelon: '[ ✔️ ]  Leveling ativado com Sucesso!',
				leveloff: '[ ❗ ]  Leveling desativado com Sucesso!',
				levelnoton: '[ ❗ ]  Leveling não está ativado!',
				levelnol: '*Level* 0 ',
				error: {
					stick: '[ ❗ ]  Erro ao converter em Figurinha!',
					Iv: '❌ Link inválido ❌'
				},
				only: {
					group: '[ ❗ ]  Este comando só funciona em grupos!',
					ownerG: '[ 💎 ]  Este comando é exclusivo para usuários Premium ou Adms! Seu membro Comum!',
					ownerB: '[ 💎 ]  Este comando é exclusivo, e só pode ser usado por Herberth!',
					admin: '[ 💎 ]  Este comando é exclusivo para usuários Premium ou Adms! Seu membro Comum!',
			 ...
			 if (isGroup && isLevelingOn) {
				...
				try {
					...
					if (requiredXp <= getLevelingXp(sender)) {
						...
						await reply(`*「 NIVEL AUMENTADO 」*\n\n➸ *Nome*: ${sender}\n➸ *XP*: ${getLevelingXp(sender)}\n➸ *Level*: ${getLevel} -> ${getLevelingLevel(sender)}\n\nParabens, por ser ativo(a) seu nivel subiu! 🎉🎉`)
					}
				} catch (err) {
					console.error(err)
				}
		...
		client.sendMessage(from, menupremium(prefix, sender), text, {quoted: mek})
				break
				case 'porn':
		client.sendMessage(from, +porn(prefix, sender), text, {quoted: mek})
                break
				case 'demote':
